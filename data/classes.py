...
class DataBase:
    ...
    def __init__(self, admin_password,
        ...
        with pg.connect(f'dbname={wished_dbname} user=postgres host=localhost password={admin_password}') as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    create table if not exists employeer_vacancy
                        (
            ...
            with conn.cursor() as cur:
                for i in self.url_list:
                    ...
                    for a in items_json['items']:
                        if "salary" in a and a["salary"] is not None and "from" in a["salary"] and a["salary"][
                            "from"] is not None:
                            anti_none_from = a["salary"]["from"]
                        else:
                            anti_none_from = 0

                        if "salary" in a and a["salary"] is not None and "to" in a["salary"] and a["salary"][
                            "to"] is not None:
                            anti_none_to = a["salary"]["to"]
                        else:
                            anti_none_to = 0

                        if "salary" in a and a["salary"] is not None and "currency" in a["salary"] and a["salary"][
                            "currency"] is not None:
                            anti_none = a["salary"]['currency']
                        else:
                            anti_none = 'Не указано'
                        cur.execute(f'INSERT INTO employeer_vacancy (id, employeer_name, vacancy_name, vacancy_description,'
                                    f'vacancy_salary_max, vacancy_salary_min, vacancy_salary_cur, '
                                    f'vacancy_url) '
                                    f'VALUES (%s, %s, %s, %s, %s, %s, %s, %s)',
                                    (a["id"], a['employer']['name'], a["name"], a["snippet"]["responsibility"], anti_none_to, anti_none_from,
    ...
    def get_companies_and_vacancies_count(self, admin_password):
        ...
        with pg.connect(f'dbname=vacancy user=postgres host=localhost password={admin_password}') as conn:
            with conn.cursor() as cur:
                cur.execute("SELECT employeer_name, count(*) FROM employeer_vacancy GROUP BY employeer_name;")
                rows = cur.fetchall()
                for row in rows:
                    print(row[0], row[1])
    ...
    def get_max_avg(self, admin_password):
        with pg.connect(f'dbname=vacancy user=postgres host=localhost password={admin_password}') as conn:
            with conn.cursor() as cur:
                cur.execute('SELECT * FROM employeer_vacancy '
                            'WHERE CAST(vacancy_salary_max AS INTEGER) > '
                            '(SELECT AVG(CAST(vacancy_salary_max AS INTEGER)) FROM employeer_vacancy)')
                cur.fetchall()
                rows = cur.fetchall()
                for row in rows:
                    print(row)
